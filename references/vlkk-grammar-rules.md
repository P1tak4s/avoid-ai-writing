...
**ė vs e:**
- `vėjas` (ne `vejas`), `sėkmė` (ne `sekme`), `pėda` (ne `peda`)
...
**Priebalsių minkštumas prieš a, o, u:**
- Rašome `i` kaip minkštumo ženklas (netariamas): `žalia`, `salia`, `šioks`
- AI klaida: `žala` vietoj `žalia`, `salas` vietoj `šalas`
...
- `naujienlaiškis` (ne `naujienų laiškis`)
- `žemėlapis` (ne `žemės lapis`)
- `tinklaraštis` (ne `tinkla-raštis`)
- `knygotyra`, `dailėtyra`, `tarmėtyra`
- `teisėkūra` (ne `teisės kūra`)
- `rinkodara` (ne `rinkos dara`)

**Priešdėliai VISADA kartu su žodžiu (be brūkšnelio):**
- `kontrkultūra` (ne `kontr-kultūra`)
- `makroaplinka` (ne `makro-aplinka`)
- `postmodernizmas` (ne `post-modernizmas`)
- `ikiinvesticinis` (ne `iki-investicinis`)
- `tarpkultūrinis` (ne `tarp-kultūrinis`)
- `poindustrinis` (ne `po-industrinis`)
...
- `Ne lengvas, o labai sunkus.`
- `ne darbo metu`, `ne konkurso tvarka`

**AI klaida:** rašo `ne kartu` kai turėtų `nekartu` arba atvirkščiai.
...
### Dalelytės rašomos atskirai
- `ar ne`, `net ir`, `vis dėlto`, `vis tiek`
- `tik ne`, `bent jau`, `kaip tik`

**AI klaida:** `visdėlto` → `vis dėlto`, `kaiptik` → `kaip tik`
...
1. Kiekvieno sakinio pirmasis žodis
2. Tikriniai vardai: `Eglė`, `Mindaugas`, `Kaunas`, `Nemunas`
3. Lietuvos Respublika (oficialus pavadinimas)
...
❌ `Greitas Ir Patikimas Remontas`
✅ `Greitas ir patikimas remontas`

**Taisyklė:** tik PIRMASIS antraštės žodis iš didžiosios (išskyrus tikriniai vardai).

### Kreipiniai laiškuose/el. laiškuose
Reiškiant pagarbą adresatui leidžiama rašyti iš didžiosios:
- `Jūs`, `Jūsų`, `Jums`, `Jūsų redakcijai`
- `Gerbiamasis`, `Tamsta`, `Pone`

**Svetainėje:** paprastai kreipiniai iš mažosios, išskyrus formalius kreipimosi tekstus.
...
**Tiesioginės reikšmės pavadinimai** (su gimininiu žodžiu):
- Vilniaus universitetas (ne kabutėse)
...
- Mažosiomis: `sausio 5 d.`, `penktadienis`
- `2024 m. gegužės 20 d.` (ne `Gegužės`)
...
**Transkribuojami (rašomi lietuviškai):**
- Vardai: `Džordžas`, `Viljamas`, `Pjeras`
- Miestai: `Niujorkas`, `Londonas`, `Paryžius`

**Rašomi originaliai (be adaptacijos):**
...
- `dizainas` (ne `design`), `marketingas` (ne `marketing`)
- `startuolis` (ne `startup`), `servisas` (ne `service`)
...
**Laiko ir datos:**
- `m.` = metai: `2024 m.`, `per 2 m.`
- `d.` = diena: `gegužės 5 d.`, `per 3 d.`
- `val.` = valanda: `8 val.`, `nuo 9 val.`
- `min.` = minutė: `per 30 min.`
- `sav.` = savaitė: `per 2 sav.`
- `mėn.` = mėnuo: `per 3 mėn.`, `50 €/mėn.`
...
**Adreso elementai:**
- `g.` = gatvė: `Gedimino g. 5`
- `pr.` = prospektas: `Konstitucijos pr. 7`
- `al.` = alėja
- `pl.` = plentas
- `a.` = aikštė
...
- Adresas: `Gedimino g. 5-12`

**Brūkšnelis NERAŠOMAS:**
- Tarp skirtingų dalykų: `gamybos ir prekybos įmonė` (NE `gamybos-prekybos`)
...
- `el. paštas` arba `e. paštas` (NE `e-paštas`)
- `el. valdžia`, `e. verslas` (NE `e-valdžia`, `e-verslas`)
- Svetainės adresas be kabutių: `revmotors.lt`
...
| `post-modernizmas` | `postmodernizmas` | Skyrius 4 |
| `gamybos-prekybos` | `gamybos ir prekybos` | Skyrius 11.1 |
| `Automobilių Servisas` | `Automobilių servisas` | Skyrius 7 |
| `vasario` (mėnuo) | `vasario` ✓ mažosiomis | Skyrius 7 |
| `ne darbas` (kai priešdėlis) | `nedarbas` | Skyrius 5 |
| `8-222-22-222` | `8 222 22 222` | Skyrius 11 |
| `startuolis` (jei reikia LT) | `startuolis` ✓ | Skyrius 9 |
| `visdėlto` | `vis dėlto` | Skyrius 5 |
| `kaiptik` | `kaip tik` | Skyrius 5 |
